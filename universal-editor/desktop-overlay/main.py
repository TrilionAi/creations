"""
Universal Editor Desktop - Drawing and annotation overlay
Works on top of any Windows program.

Controls:
- Ctrl+Num1: Toggle draw mode
- Ctrl+Num2: Toggle reading guide
- Ctrl+Num3: Toggle text mode
- Ctrl+Z: Undo last stroke
- Ctrl+Shift+C: Clear all
- Ctrl+Num0: Close application
"""
...
# Configure logging to error file (uses same directory as settings)
...
class HotkeySignals(QObject):
    """Signals for thread-safe communication between pynput and Qt threads"""
    toggle_draw = Signal()
    toggle_guide = Signal()
    toggle_text = Signal()
    undo = Signal()
...
def create_tray_icon(color='#3a86ff'):
    """Creates a pencil icon for the system tray.
    The 'color' is used as a state indicator (small circle in the corner).
    """
    size = 32
    ...
    # --- State indicator (colored circle in the bottom right corner) ---
    if color != '#3a86ff':
        indicator_size = 10
...
class EditorUniversalApp:
    """Main application for Universal Editor Desktop"""
    ...
    def __init__(self):
        ...
        self.toolbar.show()  # Shows floating button on startup

        # Config (created on demand)
        ...
        # Mouse listener (for global reading guide)
        self._setup_mouse_listener()

        # Register toolbar HWND in overlay for Z-order
        QTimer.singleShot(200, self._register_toolbar_hwnd)

        # Timer to keep overlay on top
    ...
    def _register_toolbar_hwnd(self):
        """Registers toolbar HWND in the overlay to keep toolbar on top"""
        try:
            toolbar_hwnd = int(self.toolbar.winId())
            self.overlay.set_toolbar_hwnd(toolbar_hwnd)
            print(f"[Universal Editor] Toolbar HWND registered: {toolbar_hwnd}")
        except Exception as e:
            print(f"[Universal Editor] Error registering toolbar HWND: {e}")

    def _connect_toolbar(self):
        """Connects toolbar signals to the overlay"""
    ...
    def _setup_tray(self):
        ...
        self.tray.setToolTip("Universal Editor\nRight-click for menu")
    ...
    def _setup_hotkeys(self):
        """Sets up global hotkeys with pynput"""
        ...
        self.vk_map['z'] = 90  # Z key for undo
        self.vk_map['c'] = 67
        ...
        def on_press(key):
            ...
            vk = None
            try:
                # Strategy 1: direct .vk attribute (KeyCode with numpad)
                vk = getattr(key, 'vk', None)

                # Strategy 2: Key enum -> key.value.vk
                if vk is None and hasattr(key, 'value'):
                    vk = getattr(key.value, 'vk', None)

                # Strategy 3: normal character -> ord()
    ...
    def _setup_mouse_listener(self):
        """Global mouse listener for reading guide + right-click in normal mode"""
        def on_move(x, y):
            self.signals.mouse_moved.emit(x, y)

        def on_click(x, y, button, pressed):
            if button == pynput_mouse.Button.right and pressed:
                # Right-click in normal mode -> try to select item
    ...
    def _toggle_draw(self):
        ...
        print(f"[Universal Editor] Draw mode: {'ON' if is_on else 'OFF'}")
    ...
    def _activate_pointer_mode(self):
        """Returns to normal mode (click-through)"""
        ...
        self.toolbar.update_text_mode_state(False)
        self.tray.setIcon(create_tray_icon('#3a86ff'))
        print("[Universal Editor] Normal mode (pointer)")

    def _on_tool_changed(self, tool):
        """Switches tool and re-activates overlay (toolbar stole focus on click)"""
        self.overlay.set_tool(tool)
        if self.overlay.draw_mode:
            # Delay to let Qt process the toolbar click
            QTimer.singleShot(50, self.overlay._activate_overlay)

    def _activate_draw_mode(self):
        """Activates draw mode if not already active"""
        if not self.overlay.draw_mode:
            self.overlay.toggle_draw_mode()
            self.tray.setIcon(create_tray_icon('#FF4444'))
            self.toolbar.update_text_mode_state(False)
            print("[Universal Editor] Draw mode auto-activated")

    def _handle_normal_mode_right_click(self, x, y, ctrl_held):
        """Tries to select item in normal mode (click-through temporarily disabled)"""
        ...
        if found:
            # Temporarily activate overlay for selection
    ...
    def _toggle_guide(self):
        ...
        print(f"[Universal Editor] Reading guide: {'ON' if state else 'OFF'}")

    def _toggle_text(self):
        state = self.overlay.toggle_text_mode()
        print(f"[Universal Editor] Text mode: {'ON' if state else 'OFF'}")
    ...
    def _toggle_ghost(self):
        ...
        print(f"[Universal Editor] Presentation mode: {'ON' if state else 'OFF'}")
...
def main():
    print("[Universal Editor] Starting...")

    # Single instance lock
    try:
        lock_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        lock_socket.bind(('127.0.0.1', 47834))
        print("[Universal Editor] Lock acquired (port 47833)")
    except socket.error:
        print("[Universal Editor] ERROR: Already running! Close the previous instance.")
        sys.exit(1)

    try:
        print("[Universal Editor] Creating application...")
        app = EditorUniversalApp()
        print("[Universal Editor] Ready! Use Ctrl+Num1 to draw, Ctrl+Num3 for text.")
        print("[Universal Editor] Ctrl+Num0 to close.")
        sys.exit(app.run())
    except Exception as e:
        logging.error(f"Fatal error: {e}\n{traceback.format_exc()}")
        print(f"[Universal Editor] Fatal error: {e}")
        print(f"Check the error.log file for details.")
        raise

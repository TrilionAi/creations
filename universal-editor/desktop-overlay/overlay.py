"""
Overlay - Transparent fullscreen window for drawing and annotation
Stays on top of all windows. Toggles between click-through and draw mode via hotkey.
Tools: freehand, rectangle, circle, line, arrow, eraser
Includes preview during drag and visual selection of items with handles (Photoshop-style).
Supports multi-selection (Ctrl+right-click).
"""
...
class DrawingOverlay(TextEditorMixin, HandleSystemMixin, QWidget):
    """Transparent fullscreen overlay with draw mode"""
    ...
    def __init__(self, settings=None):
        ...
        self.ghost_mode = False  # Presentation mode: drawings disappear on release
        ...
        # Unified undo stack: [('path', idx), ('text', idx), ...]
        self._undo_stack = []
    ...
    def _init_ui(self):
        """Sets up transparent fullscreen window"""
    ...
    def _cache_hwnd(self):
        try:
            ...
            # Enable click-through on initialization (normal mode)
    ...
    def _activate_overlay(self):
        """Activates the overlay to receive mouse/keyboard events"""
        self.setAttribute(Qt.WidgetAttribute.WA_ShowWithoutActivating, False)
        self.activateWindow()
        if self._hwnd:
            set_foreground(self._hwnd)
        # Toolbar ALWAYS above the overlay (overlay is fullscreen)
        # Immediate raise + delay to ensure it stabilized
        self._raise_toolbar()
    ...
    def _reactivate_if_needed(self):
        """Re-activates overlay if there is a selection or it is in active mode"""
    ...
    def toggle_guide(self):
        ...
        # Guide works in click-through -- ensure clicks pass through
        if not self.draw_mode and not self.text_mode:
            self._set_click_through(True)
        self.update()
    ...
    def paintEvent(self, event):
        ...
        # Alpha=1 for hit-testing on Windows
    ...
    def _paint_selection(self, painter):
        """Renders selection handles (single or multi)"""
        if len(self.selected_items) == 1:
            ...
            # Multi-selection: individual highlights + handles on combined bbox
    ...
    def _paint_guide(self, painter):
        ...
        # Border uses same color as guide, but more opaque
    ...
    def mousePressEvent(self, event):
        ...
        # Inline editor: commit when clicking outside
        ...
        # Handle mode: check handles first
    ...
    def _handle_right_click(self, pos, modifiers=None):
        ...
        # If has selection and clicked on a selected item (without Ctrl) -> mini menu
        if self._has_selection() and not ctrl_held:
            if self._hit_test_selected_body(pos):
                self._show_mini_context_menu(pos)
                # Re-activate overlay after closing the menu (QMenu.exec steals focus)
                QTimer.singleShot(50, self._reactivate_if_needed)
                return

        # Hit-test on all items
        for i in range(len(self.text_annotations) - 1, -1, -1):
    ...
    def mouseMoveEvent(self, event):
        ...
        # Handle transforms (uses mixin methods)
    ...
    def mouseReleaseEvent(self, event):
        # Finalize transform via handles
        ...
        # Ghost mode: drawing disappears on release (not saved)
        if self.ghost_mode:
            self.current_path = QPainterPath()
            self.update()
            return
    ...
    def undo(self):
        ...
        if item_type == 'path' and 0 <= item_index < len(self.paths):
            ...
            # Adjust remaining indices in the stack
            self._undo_stack = [
